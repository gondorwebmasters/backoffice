...
export function PromotionsByMonthChart({ data, loading }: { data: MonthCount[]; loading?: boolean }) {
  ...
  if (data.length === 0) {
    return <p className="py-16 text-center text-sm text-zinc-400">Sin promociones creadas en el período</p>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: -20 }} barCategoryGap="30%">
        ...
        <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(value) => [`${value} promociones`]} />
